...
- [x] Step 1: Create `public-catalog.ts` with the two interfaces above.
- [x] Step 2: Add the export line to `index.ts`.
- [x] Step 3: `export PATH="/c/Users/rahma/.bun/bin:$PATH" && bun run --cwd packages/shared typecheck` → PASS.
- [x] Step 4: Commit `feat(shared): add PublicPackageCardDto for public landing`.
...
- [x] Step 1: Create `dashboard.ts` with the interfaces above.
- [x] Step 2: Add the export line to `index.ts`.
...
- [x] Step 1: Write `public-packages.mapper.spec.ts` — a DB-free test that feeds a plain
...
- [x] Step 2: Run `export PATH="/c/Users/rahma/.bun/bin:$PATH" && bun run --cwd apps/api test public-packages.mapper` → FAIL (module missing).
- [x] Step 3: Implement `public-packages.mapper.ts` (`PublicPackageRow` type + `toPublicPackageCardDto` returning exactly the DTO keys; `nearestDepartureDate` = row date ? `new Date(...).toISOString()` : null).
- [x] Step 4: Implement `public-packages.service.ts` (`featured(limit=6)` using `this.tenantDb.tenantId`, the LEFT-lateral SQL above via `this.db.execute(sql\`...\`)`, map rows with `toPublicPackageCardDto`; `this.logger.info({ count }, "public.packages.listed")`).
- [x] Step 5: Run the mapper test → PASS.
- [x] Step 6: Commit `feat(api): public packages service + marketing-safe mapper`.
...
- [x] Step 1: Controller — `@Controller("public/packages")`, **no** `@UseGuards`; single
  `@Get()` returning `this.service.featured(6): Promise<PublicPackageCardDto[]>`.
- [x] Step 2: `public.module.ts` provides `PublicPackagesService`, declares the controller.
...
- [x] Step 3: Register `PublicModule` in `app.module.ts` imports.
- [x] Step 4: `bun run --cwd apps/api typecheck` → PASS.
...
- [x] Step 1: Implement the hook (query key `["public-packages"]`, `queryFn: () =>
  api.get("public/packages").json<PublicPackageCardDto[]>()`).
- [x] Step 2: `bun run --cwd apps/web typecheck` → PASS.
- [x] Step 3: Commit `feat(web): usePublicFeaturedPackages hook`.
...
- [x] Step 1: In `package-card.tsx`, change the prop type from `SearchResultDto` to a
...
- [x] Step 2: In `featured-packages.tsx`, swap `useSearchPackages({page:1,pageSize:6})` for
  `usePublicFeaturedPackages()`; the loading skeleton stays; map `data` (array, not
  `data.data`) into `PackageCard`.
- [x] Step 3: `bun run --cwd apps/web typecheck && bun run --cwd apps/web lint` → PASS.
- [x] Step 4: Smoke: with web on `:3000`→`:3002`, load `/` **logged out** → featured
...
- [x] Step 1: Write `dashboard.service.spec.ts` testing pure helpers: `daysUntil` returns
  ceil-days difference; a `toDepartureSignal(row)` mapper shape. (No DB.)
- [x] Step 2: Run test → FAIL.
- [x] Step 3: Implement `dashboard.service.ts`:
...
- [x] Step 5: Commit `feat(api): dashboard summary service`.
...
- [x] Step 1: Controller `@Controller("dashboard")` `@UseGuards(JwtAuthGuard, RolesGuard)`;
  `@Get("summary") @Roles("admin","staff")` → `this.service.summary()`.
- [x] Step 2: `dashboard.module.ts` (provider + controller); register in `app.module.ts`.
- [x] Step 3: `bun run --cwd apps/api typecheck` → PASS.
- [x] Step 4: Smoke: authed `curl` to `:3002/dashboard/summary` → `200` with counts;
  anonymous → `401`.
...
- [x] Step 1: Implement (`["dashboard-summary"]`, `api.get("dashboard/summary").json<DashboardSummaryDto>()`).
- [x] Step 2: typecheck web → PASS. Commit `feat(web): useDashboardSummary hook`.
...
- [x] Step 1: Replace the page body with: header (keep nav buttons), a KPI tile row
...
- [x] Step 2: `bun run --cwd apps/web typecheck && lint` → PASS.
- [x] Step 3: Smoke: admin `/dashboard` shows counts + ops lists; no scaffolding copy.
- [x] Step 4: Commit `feat(web): real KPI dashboard home`.

### Task 5.3: De-scaffold `users/page.tsx`

**Files:** Modify `apps/web/src/app/dashboard/users/page.tsx`.

- [x] Step 1: Remove the "The worked CRUD example - see docs/FEATURE_PATTERN.md…" subtitle
...
- [x] Step 1: Seed a tenant with one published + one draft package (draft must not appear);
...
- [x] Step 2: `bun run --cwd apps/api test:int public-packages` → PASS. Commit.

### Task 6.2: Integration — dashboard summary

**Files:** Create `apps/api/src/dashboard/dashboard.service.int.spec.ts`.

- [x] Step 1: Seed tenant A + tenant B with distinct packages/departures/providers; assert
...
- [x] Step 1: `export PATH="/c/Users/rahma/.bun/bin:$PATH" && bun run verify` → PASS.
- [x] Step 2: `bun run --cwd apps/api test:int` → PASS (all integration green).
- [x] Step 3: Live smoke (web `:3000`→`:3002`): anonymous `/` renders featured packages,
  no login redirect; admin `/dashboard` shows real counts + urgent-closing/needs-push.
- [x] Step 4: Commit any smoke-driven fixes (or note "no fixes needed").

### Task 7.2: Single code-review pass

- [x] Step 1: After ALL tasks are checked, run one `/code-review` over the whole change.
- [x] Step 2: Fix CRITICAL findings; record acceptance rationale for any non-CRITICAL
